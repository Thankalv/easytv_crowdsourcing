...
function init() {
    ...
    var loaderPromise = new Promise(function(resolve, reject) {
        function loadDone(objectMotion) {
            console.log("loader successfully completed loading task");
            mixer = new THREE.AnimationMixer( objectMotion );
            runningAction = mixer.clipAction( objectMotion.animations[ 0 ] );
            runningAction.play();
            objectMotion.traverse( function ( child ) {
                if ( child.isMesh ) {
                    child.castShadow = true;
                    child.receiveShadow = true;
                }
            });
            resolve(objectMotion); // it went ok!
        }
        var loader = new FBXLoader();
        loader.load( "../"+motionFile, loadDone);
    });
    ...
    function initScene() {
        ...
        // ground
        var mesh = new THREE.Mesh( new THREE.PlaneBufferGeometry( 2000, 2000 ), new THREE.MeshPhongMaterial( { color: 0x999999, depthWrite: false, morphtargets:true } ) );
...
function animate() {
    ...
    clock.running;
    document.getElementById("showClock").textContent = clock.elapsedTime.toFixed(1);
